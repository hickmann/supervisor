...
pub async fn secure_storage_save(app: AppHandle, items: Vec<StorageItem>) -> Result<(), String> {
    ...
    for item in items {
        match item.key.as_str() {
            "coterapia_license_key" => storage.license_key = Some(item.value),
            "coterapia_instance_id" => storage.instance_id = Some(item.value),
            "selected_coterapia_model" => storage.selected_coterapia_model = Some(item.value),
...
pub async fn secure_storage_get(app: AppHandle) -> Result<StorageResult, String> {
    ...
    Ok(StorageResult {
        ...
        selected_coterapia_model: storage.selected_coterapia_model,
    })
}
...
pub async fn secure_storage_remove(app: AppHandle, keys: Vec<String>) -> Result<(), String> {
    ...
    for key in keys {
        match key.as_str() {
            "coterapia_license_key" => storage.license_key = None,
            "coterapia_instance_id" => storage.instance_id = None,
            "selected_coterapia_model" => storage.selected_coterapia_model = None,
